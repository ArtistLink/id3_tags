...
describe 'Id3Tags.write_tags_to' do
  context 'given an M4A with all the ID3 metadata and a Hash of empty ID3' do
    let(:original_track) { asset_file('all_id3.m4a') }
    let(:new_metadata) { {} }
  ...
  context 'given an M4A with no metadata and a Hash full of ID3' do
    let(:original_track) { asset_file('no_id3.m4a') }
